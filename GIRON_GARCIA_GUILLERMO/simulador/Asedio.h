...
//
//Add @CFLAG_VISIBILITY@ or (in a Makefile.am) $(CFLAG_VISIBILITY) to the 
//CFLAGS for the compilation of the sources that make up the library.
//Add a C macro definition, say �-DBUILDING_LIBFOO�, to the CPPFLAGS for 
//the compilation of the sources that make up the library.
...
//Annotate all variable, function and class declarations in all public 
//header files of your library with �LIBFOO_DLL_EXPORTED�. This 
//annotation can occur at different locations: between the �extern� and 
//the type or return type, or just before the entity being declared, or 
//after the entire declarator. My preference is to put it right after 
//�extern�, so that the declarations in the header files remain halfway readable. 
//
//Note that the precise control of the exported symbols will not work 
//with other compilers than GCC >= 4.0, and will not work on systems where 
//the assembler or linker lack the support of �hidden� visibility. 
//Therefore, it's good if, in order to reduce the risk of collisions with 
//symbols in other libraries, you continue to use a prefix specific to your 
//library for all non-static variables and functions and for all C++ classes in 
//your library. 
